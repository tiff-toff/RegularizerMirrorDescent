...
def train(device, model, trainloader, output_dir, N, corrupted_indices, uncorrupted_indices, epochs=None, lr=None, lmbda=None, history=None, from_checkpoint=None, trial=None):
    
    total_loss = torch.zeros(epochs, device=device)
    per_sample_loss = torch.zeros(N, device=device)
    criterion = nn.CrossEntropyLoss().to(device) #losses are averaged in minibatch
    per_sample_criterion = nn.CrossEntropyLoss(reduction='none').to(device)
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=lmbda)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for images, labels, idx in trainloader:
            ...
            loss = criterion(outputs, labels)   # averaged over batch size
        ...
        with torch.no_grad():
            for images, labels, idx in trainloader:
                ...
                outputs = model(images)
                loss = criterion(outputs, labels)                 # averaged over batch size
                total_loss[epoch] += (loss.item() * len(idx))   # sum total loss
        ...
        print('Epoch [{}/{}], Loss: {:.4f}, Accuracy: {:.2f}%, Time: {:.2f}s'
              .format(epoch + 1, epochs, total_loss[epoch], (correct / total) * 100, time()-epoch_t))

        if lmbda == 0 and total == correct:
            # exit condition: 100% training accuracy for SGD
            ...
            l2_norm = torch.sum(torch.abs(w)**2)
            print('Weights: {}'.format(l2_norm))
...
if __name__=="__main__":
    ...
    if config['checkpoint.from_checkpoint']:
        print("checkpointing!")
        os.environ["OUTPUT"] = "$GROUP/rmd-experiment/sgd-weight-decay/" + str(config['checkpoint.trial'])
